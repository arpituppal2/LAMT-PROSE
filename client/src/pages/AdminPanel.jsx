                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {[['firstName','First Name'],['lastName','Last Name'],['initials','Initials']].map(([k, lbl]) => (
                          <div key={k} className={k === 'initials' ? 'col-span-1' : 'col-span-1'}>
...
const TournamentsTab = () => {
  ...
  const [editRound, setEditRound]         = useState(null); // { tournamentId, round }
      ...
      {tournaments.map(t => {
        ...
        return (
          <div key={t.id} className="surface-card overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
            ...
            <div
              className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-[var(--color-surface)] transition-colors"
              onClick={() => !isEditingT && toggleExpand(t.id)}
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                {isEditingT ? (
                  <input
                    ...
                    style={{ fontSize: 'var(--text-sm)', width: 200 }}
                  />
                ) : (
              ...
              <div className="flex items-center gap-1 flex-shrink-0 ml-3" onClick={e => e.stopPropagation()}>
                {isEditingT ? (
                  ...
                  <>
                    <button type="button" onClick={() => { setEditTId(t.id); setEditTForm({ name: t.name, description: t.description || '' }); if (!isOpen) toggleExpand(t.id); }} className="p-1.5 transition-colors" style={{ color: 'var(--color-text-faint)' }} title="Edit">
                      <Pencil size={13} />
                    </button>
            ...
            {isOpen && (
              <div className="border-t border-[var(--color-border)] px-4 py-4 space-y-3">
                {t.description && (
                  <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>{t.description}</p>
                )}
                        ...
                        {isEditR ? (
                          ...
                          <>
                            ...
                            <span className="px-2 py-0.5 border font-semibold uppercase tracking-wide"
                              style={{ fontSize: 'var(--text-xs)', background: 'var(--color-surface-offset)', borderColor: 'var(--color-border)', color: 'var(--color-text-muted)' }}>
                              {rr.roundType}
                            </span>
                ...
                <div className="flex gap-2 pt-1">
                  <input
                    type="text"
                    value={newRoundName[t.id] || ''}
                    onChange={e => setNewRoundName(p => ({ ...p, [t.id]: e.target.value }))}
                    onKeyDown={e => e.key === 'Enter' && handleAddRound(t.id)}
                    placeholder="New round name"
                    className="input-base flex-1"
                    style={{ fontSize: 'var(--text-xs)' }}
                  />
                  <select
                    value={newRoundType[t.id] || 'Individual'}
                    onChange={e => setNewRoundType(p => ({ ...p, [t.id]: e.target.value }))}
                    className="input-base"
                  ...
                  <button
                    type="button"
                    onClick={() => handleAddRound(t.id)}
                    disabled={addingRound[t.id] || !newRoundName[t.id]?.trim()}
...
const GuestContentTab = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError]   = useState('');
  const [tab, setTab]       = useState('problems'); // 'problems' | 'feedbacks'
  const [query, setQuery]   = useState('');
  ...
  const [reassignOk, setReassignOk]     = useState(null);
  ...
  const { problems = [], feedbacks = [], users = [] } = data || {};
  ...
  const filteredFeedbacks = feedbacks.filter(f =>
    !query || f.problemId.toLowerCase().includes(query.toLowerCase()) ||
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 px-4 py-3 border"
        ...
        <span>
          Content submitted by <strong>GUESTBRUINS</strong>. Reassign problems or feedbacks to the real author via their admin profile.
        </span>
      </div>

      {/* Sub-tabs */}
      <div className="flex gap-1 border-b border-[var(--color-border)]">
        {[['problems', `Problems (${problems.length})`], ['feedbacks', `Feedbacks (${feedbacks.length})`]].map(([id, lbl]) => (
          <button key={id} type="button" onClick={() => setTab(id)}
            ...
            style={{
              ...
              background: 'none', border: 'none', borderBottom: tab === id ? '2px solid var(--color-accent)' : '2px solid transparent',
              cursor: 'pointer',
            }}
          >{lbl}</button>
        ))}
      ...
      <div className="relative">
        ...
        <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search…" className="input-base w-full pl-9" style={{ fontSize: 'var(--text-sm)' }} />
      </div>

      {tab === 'problems' && (
        <div className="space-y-2">
          {filteredProblems.length === 0 && (
            <p className="text-center py-10" style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-faint)' }}>No guest problems.</p>
          )}
            ...
            <div key={p.id} className="surface-card px-4 py-3" style={{ border: '1px solid var(--color-border)' }}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 line-clamp-2" style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', fontFamily: 'var(--font-body)' }}>
      ...
      {tab === 'feedbacks' && (
        <div className="space-y-2">
          {filteredFeedbacks.length === 0 && (
            <p className="text-center py-10" style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-faint)' }}>No guest feedbacks.</p>
          )}
          {filteredFeedbacks.map(f => (
            <div key={f.id} className="surface-card px-4 py-3" style={{ border: '1px solid var(--color-border)' }}>
              <div className="flex items-start gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-mono font-semibold" style={{ fontSize: 'var(--text-xs)', color: 'var(--color-accent)' }}>→ {f.problemId}</span>
                    {f.isEndorsement && (
                      <span className="px-2 py-0.5 border font-semibold uppercase tracking-wide"
                        style={{ fontSize: 'var(--text-xs)', background: 'var(--badge-endorsed-bg)', borderColor: 'var(--badge-endorsed-border)', color: 'var(--badge-endorsed-text)' }}>
                        Endorsement
                      </span>
                    )}
                    {f.resolved && (
                      <span className="px-2 py-0.5 border font-semibold uppercase tracking-wide"
                        style={{ fontSize: 'var(--text-xs)', background: 'var(--badge-resolved-bg)', borderColor: 'var(--badge-resolved-border)', color: 'var(--badge-resolved-text)' }}>
                        Resolved
                      </span>
                    )}
                    <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-faint)' }}>{fmtDate(f.createdAt)}</span>
                  </div>
                  <p className="mt-1 line-clamp-3" style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>
                    {f.feedback || 'No text.'}
                  </p>
                </div>
              </div>
