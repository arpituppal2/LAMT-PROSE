...
const ProblemDetail = () => {
  ...
  const extractImages = (text, destination) => {
    ...
    let cleanText = text.replace(regex, (_, alt, dataUrl) => {
  ...
  const appendImageMarkdown = (baseText, images, labelPrefix) => {
    ...
    const suffix = images
      .map((img, i) => `\n\n![${labelPrefix} ${i + 1}](${img.dataUrl})`)
      .join('');
    return baseText?.trim() ? `${baseText}${suffix}` : suffix;
  };
    ...
    files.forEach(file => {
      ...
      reader.onloadend = () => {
        setEditedImages(prev => [
          ...prev,
          { dataUrl: reader.result, destination: 'problem' }
        ]);
      };
  ...
  const handleSave = async () => {
    try {
      ...
      if (problemImages.length > 0) {
        finalLatex = appendImageMarkdown(editedLatex, problemImages, 'Problem Image');
      }
      if (solutionImages.length > 0) {
        finalSolution = appendImageMarkdown(editedSolution, solutionImages, 'Solution Image');
      }
  ...
  const handleArchive = async () => {
    try {
      let updatedNotes = problem.notes;
      if (archiveReason.trim()) {
        const timestamp = new Date().toLocaleDateString();
        const reasonNote = `[Archived ${timestamp}] ${archiveReason.trim()}`;
        updatedNotes = currentNotes = problem.notes
          ? `${reasonNote}\n\n${problem.notes}`
          : reasonNote;
      ...
      <Layout>
        <div className="text-center py-20">
          ...
          <button
            ...
            className="mt-4 text-ucla-blue hover:underline flex items-center gap-2 mx-auto text-sm"
          >
            <ArrowLeft size={15} />
            Back to Inventory
          </button>
  ...
  const myId = user?.id;
  const serverUserId = problem.userId;
  const canEdit =
    problem.isAuthor ||
    problem.isAdmin ||
            ...
            {draftMeta ? (
              <div className="space-y-3 text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={loadDraftForProblem}
                    className="px-4 py-2 rounded-md bg-[#2774AE] text-white text-sm font-semibold hover:bg-[#005587]"
                  >
                    Restore draft
                  </button>
                  <button
                    type="button"
                    onClick={clearDraftForProblem}
                    className="px-4 py-2 rounded-md border border-slate-300 dark:border-white/20 text-sm font-medium"
                  >
                    Clear draft
                  </button>
                </div>
        ...
        <div className={`space-y-6 ${detailTab === 'drafts' && canEdit ? 'hidden' : ''}`}>
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="px-6 py-3 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              ...
              {isEditing && (
                <span className="text-[10px] font-semibold text-ucla-blue bg-ucla-blue/10 px-2 py-0.5 rounded uppercase">
                  Editing
                </span>
              )}
            </div>
                  ...
                  <div className="flex flex-col lg:flex-row gap-2 lg:gap-0 lg:items-stretch min-h-[280px]">
                    ...
                    >
                      {editedLatex.trim() || editedImages.some(i => i.destination === 'problem') ? (
                        <KatexRenderer
                          latex={appendImageMarkdown(
                            editedLatex,
                            editedImages.filter(i => i.destination === 'problem'),
                            'Problem Image'
                          )}
                        />
                      ) : (
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-dashed border-slate-300 dark:border-slate-700">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {editedImages.map((img, idx) => (
                        <div
                          key={idx}
                          className="relative w-24 h-28 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg overflow-hidden group"
                        >
                          ...
                          <button
                            ...
                            className={`w-full h-12 flex items-center justify-center text-[9px] font-semibold uppercase transition-colors ${
                              img.destination === 'problem'
                                ? 'bg-slate-100 dark:bg-slate-700 text-ucla-blue'
          ...
          {isEditing && (
            <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-5">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 pb-3">
                Edit Metadata
              </h3>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">
                  Solution
                  <span className="font-normal normal-case text-slate-400 ml-1">live preview on the right</span>
                </label>
                  ...
                  >
                    {editedSolution.trim() || editedImages.some(i => i.destination === 'solution') ? (
                      <KatexRenderer
                        latex={appendImageMarkdown(
                          editedSolution,
                          editedImages.filter(i => i.destination === 'solution'),
                          'Solution Image'
                        )}
                      />
                    ) : (
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2">
              Reviews
              <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2.5 py-0.5 rounded-full text-sm font-medium">
                {feedbacks.length}
              </span>
            </h2>
                      ...
                      <div className="flex justify-between items-start mb-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">
                              ...
                              <span className={`ml-2 ${
                                ...
                                {fb.isEndorsement ? 'Endorsement' : fb.resolved ? 'Resolved Review' : 'Review'}
                              </span>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {!fb.resolved && !fb.isEndorsement && canEdit && (
                            <button
                              onClick={() => {
                                setResolvingId(resolvingId === fb.id ? null : fb.id);
                                setReplyingId(null);
                              }}
                      ...
                      {isReplyingThis && (
                        <div className="mt-3 ml-4 pl-3 border-l-2 border-ucla-blue/30">
                          <p className="text-xs font-semibold text-ucla-blue dark:text-ucla-gold mb-2 flex items-center gap-1">
                            <MessageSquare size={10} />
                            {fb.authorReply ? 'Edit Reply' : 'Reply'}
                          </p>
                          <textarea
                            value={replyText}
                ...
                <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-2 text-sm font-semibold text-ucla-blue dark:text-ucla-gold">
                      <CheckCircle size={14} />
                      {previewShowSolution ? 'Hide' : 'Show'} Solution
                    </div>
                    {previewShowSolution ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="w-24 bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
                      <div
                        className="bg-ucla-blue dark:bg-ucla-gold h-full"
                        style={{ width: `${parseInt(problem.quality || 5) * 10}%` }}
                      />
                    </div>
                    <span className="text-sm font-bold text-ucla-blue dark:text-ucla-gold tabular-nums">
                      {parseInt(problem.quality || 5)}/10
                    </span>
                  </div>
                </div>
